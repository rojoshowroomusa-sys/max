...
function renderCortes(filter = "") {
  ...
  list.forEach((corte, idx) => {
    ...
    const foto = corte.img
      ...
      : `<div class="card-img"><img src="assets/cortes/${corte.id}.jpg" alt="${corte.nombre}" loading="lazy" onerror="this.parentNode.classList.add('placeholder');this.parentNode.innerHTML='<span>&#129385;</span>'" /></div>`;
    card.innerHTML = `
      ${foto}
      ...
      const bricksBuilder = await bricks.create(
        ...
        "card-form-container",
        {
          initialization: {
            ...
            // El monto se toma de la preferencia (no se envía `amount`, que es
            // incompatible con preferenceId).
        ...
        callbacks: {
          ...
          onError(error) {
            console.error("[mp] Error del Brick:", error);
            setMpStatus(
              `No se pudo mostrar el formulario de pago: ${error?.message || "error del SDK"}. Podés usar WhatsApp.`,
              "error",
            );
...
const mpPayBtnEl = document.getElementById("mpPayBtn");
if (mpPayBtnEl) {
  mpPayBtnEl.addEventListener("click", initMercadoPago);
}
