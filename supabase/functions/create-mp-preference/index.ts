...
serve(async (req) => {
  ...
  for (const { slug, qtyKg } of requested) {
    ...
    mpItems.push({
      ...
      title: prod.name,
      quantity: qtyKg,
      unit_price: Number(prod.price_per_kg),
      currency_id: "ARS",
    });
  }

  // 4) Prepara el body de la preferencia.
  ...
  // 5) Devuelve el preference id y el init_point
  return json({ id: mpData.id, init_point: mpData.init_point });
});